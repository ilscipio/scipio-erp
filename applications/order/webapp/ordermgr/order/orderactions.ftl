      ...
      <ul class="button-group">
        <#if security.hasEntityPermission("FACILITY", "_CREATE", session) && ((orderHeader.statusId == "ORDER_APPROVED") || (orderHeader.statusId == "ORDER_SENT"))>
          ...
          <#if orderHeader.orderTypeId == "SALES_ORDER">
            ...
            <a href="javascript:document.quickShipOrder.submit()" class="button tiny">${uiLabelMap.OrderQuickShipEntireOrder}</a></li>
          <#else> <#-- PURCHASE_ORDER -->
              ...
              <#if !allShipments?has_content>
                  ...
                  <li>
                    <form name="receivePurchaseOrderForm" action="/facility/control/quickShipPurchaseOrder?externalLoginKey=${externalLoginKey}" method="post">
                      ...
                      <a href="javascript:document.receivePurchaseOrderForm.submit()" class="button tiny">${uiLabelMap.CommonReceive}</a>
                  </li>
              <#else>
                  <li>
                    ...
                    <a href="javascript:document.receiveInventoryForm.submit()" class="button tiny">${uiLabelMap.OrderQuickReceivePurchaseOrder}</a>
                  </li>
                  <li>
                    ...
                    <a href="javascript:document.partialReceiveInventoryForm.submit()" class="button tiny">${uiLabelMap.CommonReceive}</a>
                  </li>
              </#if>
        ...
        <#if orderHeader.statusId != "ORDER_COMPLETED" && orderHeader.statusId != "ORDER_CANCELLED">
          <li><a href="<@ofbizUrl>OrderDeliveryScheduleInfo?orderId=${orderId}</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderViewEditDeliveryScheduleInfo}</a></li>
        </#if>
        <#if security.hasEntityPermission("ORDERMGR", "_RETURN", session) && orderHeader.statusId == "ORDER_COMPLETED">
          <#if returnableItems?has_content>
            ...
            <a href="javascript:document.quickRefundOrder.submit()" class="button tiny">${uiLabelMap.OrderQuickRefundEntireOrder}</a>
            </li>
            <li>
            ...
            <a href="javascript:document.quickreturn.submit()" class="button tiny">${uiLabelMap.OrderCreateReturn}</a>
            </li>
          </#if>
        </#if>
        ...
        <#if orderHeader?has_content && orderHeader.statusId != "ORDER_CANCELLED">
          <#if orderHeader.statusId != "ORDER_COMPLETED">
            <#--
              <li><a href="<@ofbizUrl>cancelOrderItem?${paramString}</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderCancelAllItems}</a></li>
            -->
            <li><a href="<@ofbizUrl>editOrderItems?${paramString}</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderEditItems}</a></li>
            <li>
            <form name="createOrderItemShipGroup" method="post" action="<@ofbizUrl>createOrderItemShipGroup</@ofbizUrl>">
              <input type="hidden" name="orderId" value="${orderId}"/>
            </form>
            <a href="javascript:document.createOrderItemShipGroup.submit()" class="button tiny">${uiLabelMap.OrderCreateShipGroup}</a>
            </li>
          </#if>
          <li><a href="<@ofbizUrl>loadCartFromOrder?${paramString}&amp;finalizeMode=init</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderCreateAsNewOrder}</a></li>
          <#if orderHeader.statusId == "ORDER_COMPLETED">
            <li><a href="<@ofbizUrl>loadCartForReplacementOrder?${paramString}</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderCreateReplacementOrder}</a></li>
          </#if>
        </#if>
        <li><a href="<@ofbizUrl>OrderHistory?orderId=${orderId}</@ofbizUrl>" class="button tiny">${uiLabelMap.OrderViewOrderHistory}</a></li>
        <li><a href="<@ofbizUrl>order.pdf?orderId=${orderId}</@ofbizUrl>" target="_blank" class="button tiny">PDF</a></li>
      </ul>
      
      
      <ul class="button-group">
                <#if currentStatus.statusId == "ORDER_APPROVED" && orderHeader.orderTypeId == "SALES_ORDER">
                  <li><a href="javascript:document.PrintOrderPickSheet.submit()" class="button tiny">${uiLabelMap.FormFieldTitle_printPickSheet}</a>
                ...
                <#if currentStatus.statusId == "ORDER_CREATED" || currentStatus.statusId == "ORDER_PROCESSING">
                  <li><a href="javascript:document.OrderApproveOrder.submit()" class="button tiny">${uiLabelMap.OrderApproveOrder}</a>
                ...
                <#elseif currentStatus.statusId == "ORDER_APPROVED">
                  <li><a href="javascript:document.OrderHold.submit()" class="button tiny">${uiLabelMap.OrderHold}</a>
                ...
                <#if currentStatus.statusId != "ORDER_COMPLETED" && currentStatus.statusId != "ORDER_CANCELLED">
                  <li><a href="javascript:document.OrderCancel.submit()" class="button tiny">${uiLabelMap.OrderCancelOrder}</a>
                ...
                <#if setOrderCompleteOption>
                  <li><a href="javascript:document.OrderCompleteOrder.submit()" class="button tiny">${uiLabelMap.OrderCompleteOrder}</a>
