            ...
            <#list quantitySummaryByFacility.values() as quantitySummary>
                <#if quantitySummary.facilityId??>
                    ...
                    <@tr valign="middle">
                        ...
                        <a href="<@serverUrl>/facility/control/ReceiveInventory?facilityId=${facilityId}&amp;productId=${productId}<#if externalLoginKey??>&amp;externalLoginKey=${externalLoginKey}</#if></@serverUrl>" class="${styles.link_nav!} ${styles.action_receive!}">${uiLabelMap.ProductInventoryReceive}</a></@td>
