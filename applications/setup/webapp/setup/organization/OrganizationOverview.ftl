...
<#assign htmlSectionTitle>${uiLabelMap.CommonOverview}<#rt/>
  <#lt/> (<@setupExtAppLink uri="/partymgr/control/viewprofile?partyId=${raw(partyId!)}" text=uiLabelMap.CommonManage/>)</#assign>
...
<@section title=sectionTitle relHeadingLevel=+1>
  ...
  <#macro menuContent menuArgs={}>
    <@menu args=menuArgs>
        <@menuitem type="link" href=makePageUrl("editcontactmech?partyId=${raw(partyId)}") text=uiLabelMap.CommonNew class="+${styles.action_nav!} ${styles.action_add!}"/>
    </@menu>
  </#macro>
