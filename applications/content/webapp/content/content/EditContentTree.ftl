...
<form name="editContentTree" action="<#if parameters.rename?has_content><@ofbizUrl>updateDocumentTree</@ofbizUrl><#else><@ofbizUrl>addDocumentToTree</@ofbizUrl></#if>" method="post">
    ...
    <#if parameters.rename?has_content>
        ...
        <br/ <a class="button tiny" href="javascript:document.editContentTree.submit();">${uiLabelMap.ContentRenameFolder}</a><a class="button tiny" href="<@ofbizUrl>navigateContent</@ofbizUrl>">${uiLabelMap.CommonCancel}</a>
    <#else>
        <div class="h3">${uiLabelMap.ContentNewFolder}</div>
        ${uiLabelMap.ContentRoot} : ${content.contentName}
        <input type="text" name="contentName" />
        <br /><a class="button tiny" href="javascript:document.editContentTree.submit();">${uiLabelMap.CommonCreate}</a><a class="button tiny" href="<@ofbizUrl>navigateContent</@ofbizUrl>">${uiLabelMap.CommonCancel}</a>
    </#if>
