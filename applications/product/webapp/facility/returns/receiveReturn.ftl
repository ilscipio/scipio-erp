...
<@section title=sectionTitle menuContent=menuContent>
        ...
        <#if receivedItems?has_content>
          <@section>
          <p>${uiLabelMap.ProductReceiptForReturn} ${uiLabelMap.CommonNbr}<a href="/ordermgr/control/returnMain?returnId=${returnHeader.returnId}${externalKeyParam!}" class="${styles.link_id!}">${returnHeader.returnId}</a></p>
          ...
          <#else>
            <@table type="data-complex" cellspacing="0"> <#-- orig: class="basic-table" -->
                <@tr>
                  <@td>
                    <@heading>
                      ${uiLabelMap.ProductReceiveReturn} <a href="/ordermgr/control/returnMain?returnId=${returnHeader.returnId}${externalKeyParam!}" class="${styles.link_id!}">#${returnHeader.returnId}</a>
                      <#if parameters.shipmentId?has_content>${uiLabelMap.ProductShipmentId} <a href="<@ofbizUrl>ViewShipment?shipmentId=${parameters.shipmentId}</@ofbizUrl>" class="${styles.link_id!}">${parameters.shipmentId}</a></#if>
                    </@heading>
                  </@td>
                  <@td align="right">
                      ...
                      <@table type="fields" cellspacing="0"> <#-- orig: class="basic-table" -->
                        <@tr>
                          ...
                          <#if orderItem.productId??>
                            ...
                            <@td width="45%">
                                ${returnItem.returnItemSeqId}:&nbsp;<a href="/catalog/control/EditProduct?productId=${product.productId}${externalKeyParam!}" target="catalog" class="${styles.link_idname!}">${product.productId}&nbsp;-&nbsp;${product.internalName!}</a> : ${product.description!}
