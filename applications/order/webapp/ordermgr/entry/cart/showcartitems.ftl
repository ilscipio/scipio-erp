...
<#macro showAssoc productAssoc>
  ...
  <#if assocProduct?has_content>
    <@td><a href="<@ofbizUrl>/product?product_id=${productAssoc.productIdTo}</@ofbizUrl>"class="${styles.link_record_id!}">${productAssoc.productIdTo}</a></@td>
...
<@section title="${uiLabelMap.OrderOrderItems}">
  <#if (shoppingCartSize > 0)>
    <form method="post" action="<@ofbizUrl>modifycart</@ofbizUrl>" name="cartform">
      ...
      <@table type="data-complex" autoAltRows=true cellspacing="0" cellpadding="1" border="0"> <#-- orig: class="basic-table" -->
        <@thead>
        <@tr>
          <@th>&nbsp;</@th>
          <@th colspan="2">
        ...
        <#list shoppingCart.items() as cartLine>
          ...
          <@tr><@td colspan="2">
                  <#if cartLine.getProductId()??>
                    <#-- product item -->
                    <a href="<@ofbizUrl>product?product_id=${cartLine.getProductId()}</@ofbizUrl>" class="${styles.link_record_id!}">${cartLine.getProductId()}</a> -
                   ...
                   <#if features?has_content>
                    ...
                    <#if cartLine.getAttribute("surveyResponses")?has_content>
                       ...
                       <#list cartLine.getAttribute("surveyResponses") as surveyResponseId>
                        <a href="/content/control/ViewSurveyResponses?surveyResponseId=${surveyResponseId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.link_record_id!}" style="font-size: xx-small;">${surveyResponseId}</a>
                       </#list>
                    </#if>
            </@td></@tr>
            ...
            <#if cartLine.getProductId()??>
              ...
              <@tr>
                <@td colspan="2">
                    <a href="/catalog/control/EditProductInventoryItems?productId=${productId}" class="${styles.link_action!}"><b>${uiLabelMap.ProductInventory}</b></a> : 
            ...
            <#if itemProductAssocList?? && itemProductAssocList?has_content>
              ...
              <@tr>
                ...
                <@td><a href="<@ofbizUrl>LookupAssociatedProducts?productId=${cartLine.getProductId()!}</@ofbizUrl>" class="${styles.link_action!}">${uiLabelMap.OrderQuickLookup}</a></@td>
              </@tr>
              ...
              <#list itemProductAssocList! as itemProductAssoc>
                ...
                <#if (cartLine.getIsPromo() && cartLine.getAlternativeOptionProductIds()?has_content)>
                  ...
                  <#list cartLine.getAlternativeOptionProductIds() as alternativeOptionProductId>
                    ...
                    <div><a href="<@ofbizUrl>setDesiredAlternateGwpProductId?alternateGwpProductId=${alternativeOptionProductId}&amp;alternateGwpLine=${cartLineIndex}</@ofbizUrl>" class="${styles.link_action_long!}">Select: ${alternativeOptionName?default(alternativeOptionProductId)}</a></div>
                  </#list>
                </#if>
            </@td>
              ...
              <#if adjustmentType.get("orderAdjustmentTypeId",locale) != 'SHIPPING_CHARGES'>
                <@tr>
                  <@td colspan="6" nowrap="nowrap" align="right">
                    ...
                    <#if cartAdjustment.productPromoId?has_content><a href="<@ofbizUrl>showPromotionDetails?productPromoId=${cartAdjustment.productPromoId}</@ofbizUrl>" class="${styles.link_action!}">${uiLabelMap.CommonDetails}</a></#if>:
                  </@td>
