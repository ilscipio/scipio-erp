...
<#if (orderHeader.orderId)?has_content><#-- Scipio: Only if order not yet placed -->
  <#if !printable>
    <@menu args=menuArgs>
      ...
      <@menuitem type="link" href=makeOfbizUrl({"uri":"order.pdf?orderId=" + (orderHeader.orderId)!, "fullPath":true}) target="_BLANK" class="+${styles.action_export!}" text="${rawString(uiLabelMap.CommonInvoice)} (${rawString(uiLabelMap.CommonPdf)})" />
      <#--</#if>-->
