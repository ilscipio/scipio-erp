...
<@section title="${uiLabelMap.OrderReturnsCurrent}" menuContent=menuContent>
  <#if returnList?has_content>
    <@table type="data-list" cellspacing="0"> <#-- orig: class="basic-table" -->
      ...
      <@tr>
        <@td><a href="<@ofbizUrl>returnMain?returnId=${returnHeader.returnId}</@ofbizUrl>" class="${styles.link_record_id!}">${returnHeader.returnId}</a></@td>
        <@td>${returnHeader.entryDate.toString()}</@td>
        <@td>
          <#if returnHeader.fromPartyId??>
            <a href="${customerDetailLink}${returnHeader.fromPartyId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.link_record_id!}">${returnHeader.fromPartyId}</a>
          <#else>
