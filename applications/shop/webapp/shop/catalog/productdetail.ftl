...
<@section title="">
    <@row>
        ...
        <@cell columns=6>
            <div id="product-info">
                <p>${productContentWrapper.get("DESCRIPTION","html")?if_exists}</p>
                <#-- for prices:
                ...
                <#if price.isSale?exists && price.isSale>
                    <span id="product-sale">${uiLabelMap.OrderOnSale}!</span>
                </#if>
            ...
            <form method="post" action="<@ofbizUrl>additem</@ofbizUrl>" name="addform"  style="margin: 0;">
                ...
                <fieldset>
                    ...
                    <#if product.isVirtual?if_exists?upper_case == "Y">
                        <#if product.virtualVariantMethodEnum?if_exists == "VV_FEATURETREE" && featureLists?has_content>
                            ...
                            <div class="product-quantity" style="display:none;">
                                <span><input name="quantity" value="1" size="4" maxLength="4" type="text"/><a href="javascript:javascript:addItem();" >${uiLabelMap.CommonQuantity}:</a></span>&nbsp;
                            </div>
                            <div class="product-quantity" style="display:block;>
                                <span><input type="text" size="5" value="1" disabled="disabled" /><a href="javascript:alert('Please select all features first');">${uiLabelMap.CommonQuantity}:</a></span>&nbsp;
                            </div>
                        </#if>
                    ...
                    <#if product.introductionDate?exists && nowTimestamp.before(product.introductionDate)>
                        <div class="product-not-yet-made-available">${uiLabelMap.ProductProductNotYetMadeAvailable}.</div>
                        <#-- check to see if salesDiscontinuationDate has passed -->
                    <#elseif product.salesDiscontinuationDate?exists && nowTimestamp.after(product.salesDiscontinuationDate)>
                        <div class="product-no-longer-available">${uiLabelMap.ProductProductNoLongerAvailable}.</div>
                    ...
                    <#elseif product.virtualVariantMethodEnum?if_exists != "VV_FEATURETREE">
                        <#if product.productTypeId?if_exists != "FINDIG_GOOD" && product.productTypeId?if_exists != "DIGITAL_GOOD">
                             <#if inStock>
                                 ...
                                 <div id="add_amount" class="product-quantity ${hiddenStyle}">
                                 ...
                                 <#else> 
                                     ...
                                     <a href="javascript:addItem()" id="addToCart" name="addToCart" <#if urlFile?has_content>href="javascript:addItem()"<#else>href="javascript:void(0);" class="disabled"</#if>>${uiLabelMap.OrderAddToCart}</a>
                             ...
                             <#else>
                                 <#if productStore?exists>
                                     ...
                                     <#else>
                                         ...
                                         <a href="javascript:void(0);" disabled="disabled" >${uiLabelMap.OrderAddToCart}</a><br />
                                         <span>${uiLabelMap.ProductItemOutOfStock}<#if product.inventoryMessage?exists>&mdash; ${product.inventoryMessage}</#if></span>
                                     </#if>
                                 </#if>
                             </#if>
                        <#else>
                            <a <#if urlFile?has_content>href="javascript:addItem()"<#else>href="javascript:void(0);" class="disabled"</#if> id="addToCart" name="addToCart" >${uiLabelMap.OrderAddToCart}</a>
                        </#if>
                    </#if>
    ...
    <div class="span-6 prepend-1 listing">
    ...
    <div class="span-12 prepend-1 listing">
        <div class="product-core">
    
            <#-- Long description of product -->
            <div id="product-description">
                <p>${productContentWrapper.get("LONG_DESCRIPTION","html")?if_exists}</p>
                <#assign warnings=productContentWrapper.get("WARNINGS","html")?trim/>
                <#if warnings?has_content><@alert type="warning">${warnings}</@alert></#if>
    ...
    <div class="span-6 prepend-1 listing product-associations">
         <#-- special cross/up-sell area using commonFeatureResultIds (from common feature product search) -->
        <div id="product-related">
            <#if comsmonFeatureResultIds?has_content>            
                <h3 class="">${uiLabelMap.ProductSimilarProducts}</h3>
