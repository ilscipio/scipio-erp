...
<#macro displayReturnAdjustment returnAdjustment adjEditable>
    ...
    <@tr>
       ...
       <#if (adjEditable)>
         <@td align='right'><a href='javascript:document.removeReturnAdjustment_${rowCountForAdjRemove}.submit()' class='${styles.link_action_sys!} ${styles.action_remove!}'>${uiLabelMap.CommonRemove}</a></@td>
       <#else>
       <@td>&nbsp;</@td>
       </#if>
        <#if (adjEditable)>
  ...
  <#if !requestParameters.orderId?? && returnHeader?has_content>
          ...
          <#if returnItems?has_content>
            ...
            <#list returnItems as item>
              ...
              <@tr valign="middle" alt=alt_row>
                <@td><a href="<@ofbizUrl>orderview?orderId=${item.orderId}</@ofbizUrl>" class="${styles.link_nav_record_id!}">${item.orderId}</a> - ${item.orderItemSeqId?default("N/A")}
                ...
                <@td>
                    <#if item.get("productId")??>
                        <a href="/catalog/control/EditProductInventoryItems?productId=${item.productId}" class="${styles.link_nav_record_id!}">${item.productId}</a>
                    <#else>
                        N/A
                    </#if></@td>
                <@td>
                ...
                <@td>
                    ...
                    <br />${uiLabelMap.OrderTotalQuantityReceive}: ${item.receivedQuantity}
                        <#list shipmentReceipts! as shipmentReceipt>
                            <br />${uiLabelMap.OrderQty}: ${shipmentReceipt.quantityAccepted}, ${shipmentReceipt.datetimeReceived}, <a href="/facility/control/EditInventoryItem?inventoryItemId=${shipmentReceipt.inventoryItemId}" class="${styles.link_nav_record_id!}">${shipmentReceipt.inventoryItemId}</a>
                        </#list>
                    </#if>
                    </@td>
                <@td>
                  ...
                  <#if returnHeader.statusId == "RETURN_COMPLETED" || returnHeader.statusId == "SUP_RETURN_COMPLETED">
                    ...
                    <#if itemResp?has_content>
                      <#if itemResp.paymentId?has_content>
                        <div>${uiLabelMap.AccountingPayment} ${uiLabelMap.CommonNbr}<a href="/accounting/control/paymentOverview?paymentId=${itemResp.paymentId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.link_nav_record_id!}">${itemResp.paymentId}</a></div>
                      <#elseif itemResp.replacementOrderId?has_content>
                        <div>${uiLabelMap.OrderOrder} ${uiLabelMap.CommonNbr}<a href="<@ofbizUrl>orderview?orderId=${itemResp.replacementOrderId}</@ofbizUrl>" class="${styles.link_nav_record_id!}">${itemResp.replacementOrderId}</a></div>
                      <#elseif itemResp.billingAccountId?has_content>
                        <div>${uiLabelMap.AccountingAccountId} ${uiLabelMap.CommonNbr}<a href="/accounting/control/EditBillingAccount?billingAccountId=${itemResp.billingAccountId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.link_nav_record_id!}">${itemResp.billingAccountId}</a></div>
                      </#if>
                    <#else>
                ...
                <#if returnHeader.statusId == "RETURN_REQUESTED" || returnHeader.statusId == "SUP_RETURN_REQUESTED">
                  <@td align='right'><a href='javascript:document.removeReturnItem_${item_index}.submit()' class='${styles.link_action_sys!} ${styles.action_remove!}'>${uiLabelMap.CommonRemove}</a></@td>
                <#else>
                  <@td>&nbsp;</@td>
                </#if>
              </@tr>
        ...
        <form name="returnItems" method="post" action="<@ofbizUrl>returnItems</@ofbizUrl>">
          <input type="hidden" name="returnId" value="${returnId}" />
            ...
            <#else>
              <@tr>
                <@td colspan="4" nowrap="nowrap">${uiLabelMap.OrderNoOrderFoundForParty}: <a href="${customerDetailLink}${partyId?default('_NA_')}" class="${styles.link_nav_record_id!}">${partyId?default('[null]')}</a></@td>
              </@tr>
              <@field type="generic" label="${uiLabelMap.OrderOrderId}" tooltip="${uiLabelMap.OrderReturnLoadItems}">
