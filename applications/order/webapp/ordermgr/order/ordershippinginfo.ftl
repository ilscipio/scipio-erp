...
<#if shipGroups?has_content && (!orderHeader.salesChannelEnumId?? || orderHeader.salesChannelEnumId != "POS_SALES_CHANNEL")>
  ...
  <@section title="${uiLabelMap.OrderShipmentInformation}" menuHtml=menuHtml>
      <@table type="data-complex" cellspacing="0" class="${styles.table_default!}" role="grid">
        <@thead>
          <@tr class="header-row">
          ...
          <@tr>
            <@td colspan="5">
              <form method="post" action="<@ofbizUrl>UpdateOrderItemShipGroupAssoc?view=OISGA</@ofbizUrl>" name="UpdateOrderItemShipGroupAssoc${index}"/>
                ...
                <@table type="data-complex" cellspacing="0" class="${styles.table_default!}" border="0" style="border:0">
