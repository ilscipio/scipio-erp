  ...
  <@tbody>
    <#list productStoreCatalogList as pscat>
      <@tr>
        ...
        <@td><a href="<@ofbizInterWebappUrl extLoginKey=true>/catalog/control/EditProdCatalog?prodCatalogId=${cat.prodCatalogId}</@ofbizInterWebappUrl>" target="_blank"><#if cat.catalogName?has_content>${cat.catalogName} [${cat.prodCatalogId}]<#else>${cat.prodCatalogId}</#if></a></@td>
...
<#list productStoreCatalogList as pscat>
  <@form method="get" action=makeOfbizUrl("setupCatalog") id=("setupCatalog-editCatalog-"+rawString(pscat.prodCatalogId))>
    ...
    <@field type="hidden" name="setupAdvance" value="N"/>
