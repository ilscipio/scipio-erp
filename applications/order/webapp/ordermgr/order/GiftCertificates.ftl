    ...
    <#if surveyId?has_content>
      ...
      <#if giftCardProductList?has_content>
        ...
        <#list giftCardProductList?sort_by("price") as giftCardProduct>
          <@field type="input" name="add_product_id" label="${raw(giftCardProduct.productId!)} : ${raw(giftCardProduct.productName!)}" id="productId_${giftCardProduct.price!}" value=(giftCardProduct.productId!) checked=true />
        </#list>
