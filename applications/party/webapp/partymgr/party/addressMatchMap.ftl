...
<@section id="address-match-map" title="${uiLabelMap.PageTitleCreateAddressMatchMap}" menuContent=menuContent>
  ...
  <form name="addaddrmap" method="post" action="<@ofbizUrl>createAddressMatchMap</@ofbizUrl>">
    ...
    <@field type="submitarea">
      <a href="javascript:document.addaddrmap.submit()" class="${styles.link_action!}">${uiLabelMap.CommonCreate}</a>
    </@field>
  </form>
  </@section>
        ...
        <@table type="data-list" autoAltRows=true class="+hover-bar" cellspacing="0"> <#-- orig: class="basic-table hover-bar" -->
          ...
          <#list addressMatchMaps as map>
            <@tr>
              ...
              <@td class="button-col"><a href="<@ofbizUrl>removeAddressMatchMap?mapKey=${map.mapKey}&amp;mapValue=${map.mapValue}</@ofbizUrl>" class="${styles.link_action!}">${uiLabelMap.CommonDelete}</a></@td>
            </@tr>
          </#list>
          </@tbody>
        </@table>
