...
<#if !requestParameters.orderId?? && returnHeader?has_content>
        ...
        <@table type="data" useAltRows=false cellspacing="0" class="basic-table">
            ...
            <#list returnItems as item>
              ...
              <@tr valign="middle" alt=alt_row>
                  ...
                  <#if returnHeader.statusId == "RETURN_COMPLETED" || returnHeader.statusId == "SUP_RETURN_COMPLETED">
                    ...
                    <#if itemResp?has_content>
                      <#if itemResp.paymentId?has_content>
                        <div>${uiLabelMap.AccountingPayment} ${uiLabelMap.CommonNbr}<a href="/accounting/control/paymentOverview?paymentId=${itemResp.paymentId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.button_default!}">${itemResp.paymentId}</a></div>
                      <#elseif itemResp.replacementOrderId?has_content>
                        <div>${uiLabelMap.OrderOrder} ${uiLabelMap.CommonNbr}<a href="<@ofbizUrl>orderview?orderId=${itemResp.replacementOrderId}</@ofbizUrl>" class="${styles.button_default!}">${itemResp.replacementOrderId}</a></div>
                      <#elseif itemResp.billingAccountId?has_content>
                        <div>${uiLabelMap.AccountingAccountId} ${uiLabelMap.CommonNbr}<a href="/accounting/control/EditBillingAccount?billingAccountId=${itemResp.billingAccountId}${StringUtil.wrapString(externalKeyParam)}" class="${styles.button_default!}">${itemResp.billingAccountId}</a></div>
                      </#if>
                    <#else>
                ...
                <#if returnHeader.statusId == "RETURN_REQUESTED" || returnHeader.statusId == "SUP_RETURN_REQUESTED">
                  <@td align='right'><a href='javascript:document.removeReturnItem_${item_index}.submit()' class='${styles.button_default!}'>${uiLabelMap.CommonRemove}</a></@td>
                <#else>
                  <@td>&nbsp;</@td>
                </#if>
              </@tr>
