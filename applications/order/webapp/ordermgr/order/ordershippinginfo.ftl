    ...
    <#else>
      <#macro menuContent menuArgs={}>
          <@menu args=menuArgs>
              ...
              <#if (shipGroup.shipmentMethodTypeId?has_content && shipGroup.shipmentMethodTypeId != "NO_SHIPPING")>
                  ...
                  <#if (orderHeader.statusId == "ORDER_APPROVED" || orderHeader.statusId == "ORDER_CREATED")>
                      <#assign pickPackParams = "facilityId=" + (storeFacilityId!) + "&orderId=" + (orderId!) + "&shipGroupSeqId=" + (shipGroup.shipGroupSeqId!) + raw(externalKeyParam)>

                      <#assign pendingPickShipments = Static["org.ofbiz.entity.util.EntityUtil"].filterByOr(shipments, [{"statusId" : "SHIPMENT_INPUT"}, {"statusId" : "SHIPMENT_SCHEDULED"}])>
                      <#if !shipments?has_content || pendingPickShipments?has_content>
                        <#if pendingPickShipments?has_content>
                            <#assign pickPackParams += "&shipmentId=" + pendingPickShipments?first.shipmentId!>
                        </#if>
                        <@menuitem type="link" href=makeServerUrl("/facility/control/VerifyPick?" + pickPackParams) text=uiLabelMap.OrderPickShipmentForShipGroup class=("+" + (styles.action_run_sys!) + " " + (styles.action_find!)) />
                      </#if>

                      <#assign pendingPackShipments = Static["org.ofbiz.entity.util.EntityUtil"].filterByOr(shipments, [{"statusId" : "SHIPMENT_INPUT"}, {"statusId" : "SHIPMENT_SCHEDULED", "statusId" : "SHIPMENT_PICKED"}])>
                      <#if !shipments?has_content || pendingPackShipments?has_content>
                        <#if pendingPackShipments?has_content>
                            <#assign pickPackParams += "&shipmentId=" + pendingPackShipments?first.shipmentId!>
                        </#if>
                        <@menuitem type="link" href=makeServerUrl("/facility/control/PackOrder?" + pickPackParams) text=uiLabelMap.OrderPackShipmentForShipGroup class=("+" + (styles.action_run_sys!) + " " + (styles.action_view!)) />
                      </#if>
        ...
        <#assign allowedOrderStatus = (orderHeader?has_content && orderHeader.statusId != "ORDER_CANCELLED" && orderHeader.statusId != "ORDER_COMPLETED"
                                    && orderHeader.statusId != "ORDER_REJECTED" && orderHeader.statusId != "ORDER_SENT")!false>
        <#list shipGroups as shipGroup>
          ...
          <#assign shipments = shipGroup.getRelated("PrimaryShipment")!>
            <@section title="${uiLabelMap.OrderOrderId}: ${orderId!} - ${rawLabel('OrderShipGroup')}: ${raw(shipGroup.shipGroupSeqId)}" menuContent=menuContent>
                <@fields type="default-manual">
                    <@table type="fields">
                        <form name="updateOrderItemShipGroup" method="post" action="<@pageUrl>updateShipGroupShipInfo</@pageUrl>">
                            ...
                            <@tr>
                                ...
                                <@td valign="top" colspan="3">
                                    <@row>
                                        <#if (shipGroup.shipmentMethodTypeId?has_content && shipGroup.shipmentMethodTypeId != "NO_SHIPPING") && allowedOrderStatus>
                                            <@cell columns=6>
                                        ...
                                        <#else>
                                            <@cell columns=6>${(shipGroupAddress.address1)!"--"}</@cell>
                                        </#if>
                                        <#-- Required for newShippingAddressFunction -->
                                        <@cell columns=3 last=true>
                                            <@modal id="newShippingAddressForm_${raw(shipGroup.shipGroupSeqId)}" label=uiLabelMap.OrderNewShippingAddress linkClass=(styles.link_nav! + " " + styles.action_add!)>
                                                <@section>
                                                    <@form id="addShippingAddress" name="addShippingAddress" method="post" action="addShippingAddress">
                                                        <@fields type="default">
                                                            ...
                                                            <@field type="submitarea">
                                                                <@field type="submit" submitType="input-button" id="submitAddShippingAddress" text=uiLabelMap.CommonSubmit />
    <#--                                                            <@field type="submit" submitType="input-button" class="+popup_closebox ${styles.link_run_local!} ${styles.action_close!}" text=uiLabelMap.CommonClose />-->
                                                            </@field>
                                                        </@fields>
                                                    </@form>
                                                </@section>
                              ...
                              <@td align="left" valign="top" width="80%">
                                <#if allowedOrderStatus>
                                  <form id="updateShippingInstructionsForm_${shipGroup.shipGroupSeqId}" name="updateShippingInstructionsForm" method="post" action="<@pageUrl>setShippingInstructions</@pageUrl>">
                                    ...
                                    <@row>
                                        <@cell columns=6>
                                            <@field type="textarea" name="shippingInstructions" id="shippingInstructions_${shipGroup.shipGroupSeqId}" rows=5 inline=true>${shipGroup.shippingInstructions!}</@field>
                                        </@cell>
                                        <@cell columns=6>
                                            <@field type="submit" text=uiLabelMap.CommonUpdate class="${styles.link_run_sys!} ${styles.action_update!}" />
                                        </@cell>
                                  ...
                                  <@td>
                                      <#if allowedOrderStatus>
                                        <form id="setGiftMessageForm_${shipGroup.shipGroupSeqId}" name="setGiftMessageForm" method="post" action="<@pageUrl>setGiftMessage</@pageUrl>">
                                          ...
                                          <@row>
                                              <@cell columns=6>
                                                <@field type="textarea" name="giftMessage" id="giftMessage_${shipGroup.shipGroupSeqId}" rows=5 inline=true>${shipGroup.giftMessage!}</@field>
                                              </@cell>
                                              <@cell columns=6>
                                                <@field type="submit" text=uiLabelMap.CommonUpdate class="${styles.link_run_sys!} ${styles.action_update!}" />
                                              </@cell>
                                      ...
                                      <#else>
                                          <@cell columns=6>
                                              ...
                                              <#else>
                                                  ${uiLabelMap.OrderThisOrderNotGift}
                                              </#if>
                                          </@cell>
                                      </#if>
                                  </@td>
                             ...
                             <#if orderHeader.orderTypeId == "SALES_ORDER">
                               <#if !shipGroup.supplierPartyId?has_content>
                                   <#if shipments?has_content>
                                       <@section title=uiLabelMap.FacilityShipments>
                                           <#list shipments as shipment>
                                               <@table type="fields">
                                                   <@tr>
                                                       <@td scope="row" class="${styles.grid_large!}3">
                                                           ${uiLabelMap.FacilityShipment}
                                                       </@td>
                                                       <@td>
                                                           <#assign currentStatus = shipment.getRelatedOne("StatusItem", false) />
                                                           <a href="<@serverUrl>/facility/control/EditShipment?shipmentId=${shipment.shipmentId}</@serverUrl>">${shipment.shipmentId}</a>
                                                       </@td>
                                                   </@tr>
                                                   <@tr>
                                                       <@td scope="row" class="${styles.grid_large!}3">
                                                           ${uiLabelMap.ProductShipmentTypeId}
                                                       </@td>
                                                       <@td>
                                                           <#assign shipmentType = shipment.getRelatedOne("ShipmentType", false) />
                                                           ${shipmentType.description}
                                                       </@td>
                                                   </@tr>
                                                   <@tr>
                                                       <@td scope="row" class="${styles.grid_large!}3">
                                                           ${uiLabelMap.ProductStatusId}
                                                       </@td>
                                                       <@td>
                                                           <#assign currentStatus = shipment.getRelatedOne("StatusItem", false) />
