...
<#macro menuContent menuArgs={}>
  <@menu args=menuArgs>
    ...
    <#if shipment?has_content>
      <@menuitem type="link" href=makeOfbizUrl("EditShipment?shipmentId=${shipmentId}") text="${uiLabelMap.ProductEditShipment}" />
    </#if>
  </@menu>
</#macro>
      ...
      <#if shipmentPackage?has_content>
        ...
        <#if (0 < weight?double) && !requestParameters.reweigh??>
            ...
            <@row>
              ...
              <@cell columns=3>
                <@field type="submitarea">
                    <a href="javascript:document.routeForm.submit();" class="${styles.link_action_sys!} ${styles.action_updatestatus!}">${uiLabelMap.ProductConfirmShipmentUps}</a>
                </@field>
              </@cell>
            </@row>
            <@row>
              ...
              <@cell columns=3>
                <@field type="submitarea">
                    <a href="<@ofbizUrl>quickShipOrder?facilityId=${facilityId}&amp;shipmentId=${shipmentId}&amp;reweigh=Y</@ofbizUrl>" class="${styles.link_action_sys!} ${styles.action_update!}">${uiLabelMap.ProductReWeighPackage}</a>
                </@field>
              </@cell>
            </@row>
            <@row>
          ...
          <#else>
            ...
            <#assign allDone = "yes">
            <center>
              <a href="<@ofbizUrl>viewShipmentPackageRouteSegLabelImage?shipmentId=${requestParameters.shipmentId}&amp;shipmentRouteSegmentId=${requestParameters.shipmentRouteSegmentId}&amp;shipmentPackageSeqId=00001</@ofbizUrl>" target="_blank" class="${styles.link_action_sys!} ${styles.action_export!}">${uiLabelMap.ProductShippingLabel}</a><br />
              <a href="<@ofbizUrl>ShipmentManifest.pdf?shipmentId=${requestParameters.shipmentId}&amp;shipmentRouteSegmentId=${requestParameters.shipmentRouteSegmentId}</@ofbizUrl>" target="_blank" class="${styles.link_action_sys!} ${styles.action_export!}">${uiLabelMap.ProductPackingSlip}</a>
            </center>
          </#if>
          </@section>
        <#else>
          ...
          <form name="weightForm" method="post" action="<@ofbizUrl>setQuickPackageWeight</@ofbizUrl>">
            ...
            <input type="hidden" name="shipmentPackageSeqId" value="${shipmentPackage.shipmentPackageSeqId}"/>
              ...
              <@field type="submitarea">
                  ...
                  <a href="javascript:document.weightForm.submit();" class="${styles.link_action_sys!} ${styles.action_update!}">${uiLabelMap.ProductSetWeight}</a>
              </@field>
          </form>
          <@script>
    ...
    <form name="selectOrderForm" method="post" action="<@ofbizUrl>createQuickShipment</@ofbizUrl>">
      ...
      <input type="hidden" name="setPackedOnly" value="Y" />
        ...
        <@field type="submitarea">
            ...
            <a href="javascript:document.selectOrderForm.submit();" class="${styles.link_action_sys!} ${styles.action_update!}">${uiLabelMap.ProductShipOrder}</a>
        </@field>
    </form>
    <@script>
