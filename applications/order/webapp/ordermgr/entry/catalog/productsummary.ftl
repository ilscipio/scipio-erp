    ...
    <div class="productsummary">
        ...
        <div class="productbuy">
          ...
          <#elseif product.productTypeId! == "ASSET_USAGE">
            <a href="${productUrl}" class="${styles.button_default!}">${uiLabelMap.OrderMakeBooking}...</a>
          <#-- check to see if it is an aggregated or configurable product; will enter parameters on the detail screen-->
          <#elseif product.productTypeId! == "AGGREGATED" || product.productTypeId! == "AGGREGATED_SERVICE">
            <a href="${productUrl}" class="${styles.button_default!}">${uiLabelMap.OrderConfigure}...</a>
          <#-- check to see if the product is a virtual product -->
          <#elseif product.isVirtual?? && product.isVirtual == "Y">
            <a href="${productUrl}" class="${styles.button_default!}">${uiLabelMap.OrderChooseVariations}...</a>
          <#-- check to see if the product requires an amount -->
          <#elseif product.requireAmount?? && product.requireAmount == "Y">
            <a href="${productUrl}" class="${styles.button_default!}">${uiLabelMap.OrderChooseAmount}...</a>
            ...
            <#if mainProducts?has_content>
              ...
              <#if prodCatMem?? && prodCatMem.quantity?? && 0.00 < prodCatMem.quantity?double>
                <form method="post" action="<@ofbizUrl>additem</@ofbizUrl>" name="the${requestAttributes.formNamePrefix!}${requestAttributes.listIndex!}defaultform">
                  ...
                  <input type="hidden" name="clearSearch" value="N"/>
                  <input type="hidden" name="mainSubmitted" value="Y"/>
                  <a href="javascript:document.the${requestAttributes.formNamePrefix!}${requestAttributes.listIndex!}defaultform.submit()" class="${styles.button_default!}">${uiLabelMap.CommonAddDefault}(${prodCatMem.quantity?string.number}) ${uiLabelMap.OrderToCart}</a>
                </form>
        ...
        <div class="productinfo">
          <div>
            <a href="${productUrl}" class="${styles.link_name!}">${productContentWrapper.get("PRODUCT_NAME", "html")!}</a>
          </div>
          ...
          <a href="javascript:document.addToCompare${requestAttributes.listIndex!}form.submit()" class="${styles.button_default!}">${uiLabelMap.ProductAddToCompare}</a>
        </div>
    </div>
<#else>
