...
public final class EntityUtil {
    ...
    /**
     * SCIPIO: For each member entity alias in the given view entity, makes a GenericValue mapping the corresponding fields
     * to the member entity.
     * <p>TODO: The entities can optionally be loaded from database if available for those whose full primary is present,
     * and the resulting entities can be committed using {@link GenericValue#createOrStore()}.</p>
     */
    public static Map<String, GenericValue> makeViewEntityMemberEntities(GenericValue viewEntity, boolean loadFromDb,
                                                                         boolean alwaysMakeRelOptional, boolean orderByDependencies) {
        ModelEntity entity = viewEntity.getModelEntity();
        if (!(entity instanceof ModelViewEntity)) {
            throw new IllegalArgumentException("Not a view-entity: " + viewEntity.getEntityName());
        }
        return makeViewEntityMemberEntities(viewEntity.getDelegator(), (ModelViewEntity) entity, viewEntity, loadFromDb, alwaysMakeRelOptional, orderByDependencies);
    }
     ...
     * and the resulting entities can be committed using {@link GenericValue#createOrStore()}.</p>
     */
    public static Map<String, GenericValue> makeViewEntityMemberEntities(Delegator delegator, String viewEntity,
                                                                         Map<String, Object> fields, boolean loadFromDb,
                                                                         boolean alwaysMakeRelOptional, boolean orderByDependencies) {
        ModelEntity entity = delegator.getModelEntity(viewEntity);
        if (!(entity instanceof ModelViewEntity)) {
            throw new IllegalArgumentException("Not a view-entity: " + viewEntity);
        }
        return makeViewEntityMemberEntities(delegator, (ModelViewEntity) entity, fields, loadFromDb, alwaysMakeRelOptional, orderByDependencies);
    }
     ...
     * and the resulting entities can be committed using {@link GenericValue#createOrStore()}.</p>
     */
    public static Map<String, GenericValue> makeViewEntityMemberEntities(Delegator delegator, ModelViewEntity viewEntity,
                                                                         Map<String, Object> fields, boolean loadFromDb,
                                                                         boolean alwaysMakeRelOptional, boolean orderByDependencies) {
        Map<String, GenericValue> aliasEntities = new LinkedHashMap<>();

        if (orderByDependencies) {
            // Preset key order in the LinkedHashMap
            for(String entityAlias : viewEntity.getMemberEntityDependencyOrderByAlias(delegator.getModelReader())) {
                aliasEntities.put(entityAlias, null);
            }
        }

        if (loadFromDb) {
            // TODO: preload entities with full primary keys present in fields
        }

        // Set field value on the entity
        for(ModelViewEntity.ModelAlias alias : viewEntity.getAliases()) {
            if (!fields.containsKey(alias.getName())) {
                continue;
            }
            ModelViewEntity.ModelMemberEntity memberEntity = viewEntity.getMemberModelMemberEntity(alias.getEntityAlias());
            if (memberEntity == null) {
                continue; // coding error, should have been reported
            }
            Object value = fields.get(alias.getName());
            GenericValue entity = aliasEntities.get(alias.getEntityAlias());
            if (entity == null) {
                entity = delegator.makeValue(memberEntity.getEntityName());
                aliasEntities.put(alias.getEntityAlias(), entity);
            }
            entity.set(alias.getField(), value);
        }

        // Copy the field value to view-linked entities where needed
        // NOTE: separate loop needed due to rel-optional links
        for(ModelViewEntity.ModelAlias alias : viewEntity.getAliases()) {
            if (!fields.containsKey(alias.getName())) {
                continue;
            }
            ...
            Object value = fields.get(alias.getName());
            for(ModelViewEntity.ModelViewLink viewLink : viewEntity.getViewLinks()) {
                if (alias.getEntityAlias().equals(viewLink.getEntityAlias())) {
                    String relFieldName = viewLink.getRelFieldNameForField(alias.getField());
                    if (relFieldName == null) {
                        continue;
                    }
                    GenericValue relEntity = aliasEntities.get(viewLink.getRelEntityAlias());
                    if (relEntity == null) {
                        if (viewLink.isRelOptional() && !alwaysMakeRelOptional) {
                            continue;
                        }
                        ModelViewEntity.ModelMemberEntity relMemberEntity = viewEntity.getMemberModelMemberEntity(viewLink.getRelEntityAlias());
                        if (relMemberEntity == null) {
                            continue;
                        }
                        relEntity = delegator.makeValue(relMemberEntity.getEntityName());
                        aliasEntities.put(viewLink.getRelEntityAlias(), relEntity);
                    }
                    relEntity.set(relFieldName, value);
                } else if (alias.getEntityAlias().equals(viewLink.getRelEntityAlias())) {
                    String fieldName = viewLink.getFieldNameForRelField(alias.getField());
                    if (fieldName == null) {
                        continue;
                    }
                    GenericValue srcEntity = aliasEntities.get(viewLink.getEntityAlias());
                    if (srcEntity == null) {
                        ModelViewEntity.ModelMemberEntity srcMemberEntity = viewEntity.getMemberModelMemberEntity(viewLink.getEntityAlias());
                        if (srcMemberEntity == null) {
                            continue;
                        }
                        srcEntity = delegator.makeValue(srcMemberEntity.getEntityName());
                        aliasEntities.put(viewLink.getEntityAlias(), srcEntity);
                    }
                    srcEntity.set(fieldName, value);
                }
            }
        }
        ...
        // Remove null entries
        if (orderByDependencies) {
            Iterator<Map.Entry<String, GenericValue>> it = aliasEntities.entrySet().iterator();
            while(it.hasNext()) {
                Map.Entry<String, GenericValue> entry = it.next();
