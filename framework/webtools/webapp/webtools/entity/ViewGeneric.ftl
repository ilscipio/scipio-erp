    ...
    <#if enableEdit = "true" && ( hasUpdatePermission || hasCreatePermission)>
        <#assign alt_row = false>
          ...
          <form action='<@ofbizUrl>UpdateGeneric?entityName=${entityName}</@ofbizUrl>' method="post" name="updateForm">
            ...
            <@table type="fields" autoAltRows=true class="${styles.table_default!}" cellspacing="0">
              <#if value?has_content>
      ...
      <#else>
          <@row>
            <@cell>
              ...
              <#if value?has_content>
                <@table type="fields" autoAltRows=true class="${styles.table_default!} ${styles.grid_large!}12" cellspacing="0">
                  <@thead>
                  <@tr>
