  ...
  <form name="listOrdersImported" method="post">
      ...
      <@table type="data-list" autoAltRows=true> <#-- orig: class="basic-table hover-bar" --> <#-- orig: cellspacing="0" -->
       <@thead>
        ...
        <#if orderList?has_content>
          <#list orderList as orderHeader>
            ...
            <@tr valign="middle">
              ...
              <@td><a href="#" onclick="javascript:uploadTrackingCode('${orderHeader.orderId}','${productStoreId}')" class="${styles.link_nav_info_id!}">${orderHeader.orderId}</a></@td>
              <@td>
        ...
        <#else>
          <@tr type="meta">
            <@td colspan='4'><@commonMsg type="result-norecord">${uiLabelMap.EbayNoOrderImported}.</@commonMsg></@td>
          </@tr>
        </#if>
        <#if lookupErrorMessage??>
          <@tr type="meta">
            <@td colspan='4'><@commonMsg type="error">${lookupErrorMessage}</@commonMsg></@td>
          </@tr>
        </#if>
      </@table>
  </form>
