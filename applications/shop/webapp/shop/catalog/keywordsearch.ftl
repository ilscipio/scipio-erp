...
<#assign searchTitleMarkup>${uiLabelMap.ProductProductSearch}<#--<#if currentSearch?has_content>: ${currentSearch} </#if> (<a href="<@pageUrl>advancedsearch?SEARCH_CATEGORY_ID=${(requestParameters.SEARCH_CATEGORY_ID)!}</@pageUrl>" class="${styles.action_nav!} ${styles.action_find!}">${uiLabelMap.CommonRefineSearch}</a>)--></#assign>
...
<@row>
    <@cell columns=6>
        ${uiLabelMap.CommonSortedBy}: ${searchSortOrderString} (<a href="<@pageUrl>advancedsearch?SEARCH_CATEGORY_ID=${(requestParameters.SEARCH_CATEGORY_ID)!}</@pageUrl>" class="${styles.action_nav!} ${styles.action_find!}">${uiLabelMap.CommonRefineSearch}</a>)
    </@cell>
