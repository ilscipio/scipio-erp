<form name="demoDataGeneratorForm" method="post" action="<@ofbizUrl>DemoDataGeneratorResult?_RUN_SYNC_=Y</@ofbizUrl>">
      ...
      <@fields fieldArgs={"labelColumns":4}>
        <#list serviceParameters as serviceParameter>    
          ...
          <#else>
            <#assign fieldLabel>${serviceParameter.name} (<em>${serviceParameter.type}</em>)<#if serviceParameter.defaultValue!?string?has_content> (${uiLabelMap.WebtoolsServiceDefault}: <em>${serviceParameter.defaultValue}</em>)</#if></#assign>
