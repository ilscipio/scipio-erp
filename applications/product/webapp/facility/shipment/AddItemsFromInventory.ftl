...
<@section title="${uiLabelMap.ProductIssueInventoryItemsToShipment}: [${shipmentId!}]">
    <@table type="data-list" cellspacing="0" cellpadding="2" class="+hover-bar"> <#-- orig: class="basic-table hover-bar" -->
     <@thead>
      <@tr class="header-row">
      ...
      <#list items as item>
        <@tr>
          <@td><a href="/ordermgr/control/returnMain?returnId=${item.returnId}" class="${styles.link_id!}">${item.returnId}</a> [${item.returnItemSeqId}]</@td>
          <@td><a href="/catalog/control/EditProductInventoryItems?productId=${item.productId}" class="${styles.link_id!}">${item.productId}</a> ${item.internalName!}</@td>
          ...
          <@td>
            <#if item.issuedItems?has_content>
              <#list item.issuedItems as issuedItem>
                <div><a href="/facility/control/EditInventoryItem?inventoryItemId=${issuedItem.inventoryItemId}" class="${styles.link_id!}">${issuedItem.inventoryItemId}</a> ${issuedItem.quantity}</div>
              </#list>
            </#if>
          </@td>
          ...
          <#if (item.shipmentItemQty > item.totalQtyIssued)>
            <@td>
                <form name="issueInventoryItemToShipment_${item_index}" action="<@ofbizUrl>issueInventoryItemToShipment</@ofbizUrl>" method="post">
                  ...
                  <input type="submit" value="${uiLabelMap.CommonSubmit}" class="${styles.link_action!}"/>
                </form>
            </@td>
          </#if>
        </@tr>
