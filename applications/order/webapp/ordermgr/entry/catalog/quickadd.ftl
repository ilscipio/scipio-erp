    ...
    <@tr>
      ...
      <@td align="right">
        <form name="choosequickaddform" method="post" action="<@ofbizUrl>quickadd</@ofbizUrl>">
          ...
          <div><a href="javascript:document.choosequickaddform.submit()" class="${styles.link_action!}">${uiLabelMap.ProductChooseQuickAddCategory}</a></div>
        </form>
      </@td>
    </@tr>
...
<#if productCategoryMembers?? && 0 < productCategoryMembers?size>
  ...
  <form method="post" action="<@ofbizUrl>addtocartbulk</@ofbizUrl>" name="bulkaddform">
    ...
    <div class="quickaddall">
      <a href="javascript:document.bulkaddform.submit()" class="${styles.link_action!}">${uiLabelMap.OrderAddAllToCart}</a>
    </div>
