...
List openingCashBalanceAndExprs = new ArrayList(mainAndExprs);
...
transactionTotals = [];
balanceTotal = BigDecimal.ZERO;
List periodCashBalanceAndExprs = new ArrayList(mainAndExprs);
...
transactionTotals.addAll(new ArrayList(context.openingCashBalanceList));
transactionTotals.addAll(new ArrayList(context.periodCashBalanceList));
    ...
    transactionTotals.each { transactionTotal ->
        if (transactionTotal.D != null) {
            if (transactionTotalsMap.(transactionTotal.glAccountId)) {
                ...
                transactionTotalsMap.(transactionTotal.glAccountId) = new HashMap(transactionTotal);
            }
            accountBalanceList = UtilMisc.sortMaps(transactionTotalsMap.values().asList(), UtilMisc.toList("accountCode"));
        }
    }
