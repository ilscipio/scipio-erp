...
FIXME: Needs parameter to control injection and location of hidden modal content so can inject
  ...
  * Parameters *
    ...
    anchorClass             = ((css-class)) CSS classes or additional classes for anchor
                              Supports prefixes (see #compileClassArg for more info):
                              * {{{+}}}: causes the classes to append only, never replace defaults (same logic as empty string "")
                              * {{{=}}}: causes the classes to replace non-essential defaults (same as specifying a class name directly)  
    href                    = Link
...
<#assign modal_defaultArgs = {
  "id":"", "label":"", "href":"", "icon":"", "class":"","anchorClass":"", "passArgs":{}
}>
<#macro modal args={} inlineArgs...>
  ...
  <@modal_markup id=id modalId=modalId linkId=linkId label=label href=href class=class anchorClass=anchorClass icon=icon linkHasContent=linkHasContent origArgs=origArgs passArgs=passArgs><#nested></@modal_markup>
</#macro>

<#-- @modal main markup - theme override -->
<#macro modal_markup id="" modalId="" linkId="" label="" href="" class="" anchorClass="" icon="" linkHasContent=true origArgs={} passArgs={} catchArgs...>
  ...
  <a id="${escapeVal(linkId, 'html')}" href="#" data-reveal-id="${escapeVal(modalId, 'html')}"<#if href?has_content> data-reveal-ajax="${escapeFullUrl(href, 'html')}"</#if><#rt/>
    <#lt/><@compiledClassAttribStr anchorClass=anchorClass /><#if !linkHasContent> style="display:none;"</#if>><#if icon?has_content><i class="${escapeVal(icon, 'html')}"></i> </#if>${escapeVal(label, 'htmlmarkup')}</a>
