...
<@nav type="breadcrumbs">
    ...
    <#list crumbs as crumb>
         <#if catContentWrappers?? && catContentWrappers[crumb]??>
              <li <#if !crumb_has_next && !productContentWrapper??>class="${style_nav_breadcrumbs_active!}"</#if>>
    ...
    <#if productContentWrapper??>
          <li class="${style_nav_breadcrumbs_active!}">${productContentWrapper.get("PRODUCT_NAME")!}</li>
    </#if>
    ...
    <#if !crumbs?has_content && !productContentWrapper??>
        <li class="${style_nav_breadcrumbs_active!}"><#if (page.titleProperty)?has_content>${uiLabelMap[page.titleProperty]}<#else>${(page.title)!}</#if></li>
    </#if>
</@nav>