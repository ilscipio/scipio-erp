...
<@section>
  ...
  <#if orderContentWrapper.get("IMAGE_URL", "url")?has_content>
    <@tr>
      ...
      <@td colspan="3">
        <a href="<@ofbizUrl>viewimage?orderId=${orderId}&amp;orderContentTypeId=IMAGE_URL</@ofbizUrl>" target="_orderImage" class="${styles.button_default!}">${uiLabelMap.OrderViewImage}</a>
      </@td>
    </@tr>
  </#if>
  ...
  <#if "SALES_ORDER" == orderHeader.orderTypeId>
    ...
    <@tr>
      ...
      <@td colspan="3">
         ...
         <input type = "hidden" name="orderId" value="${orderId}"/>
        <@input type="select" name="invoicePerShipment">
